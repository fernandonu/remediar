...
function form_busqueda($sql, $orden, $filtro, $link_pagina, $where_extra = "", $contar =
  ...
  if ($page > 0) {
    ...
    $link_pagina_p = "<a title='Pagina anterior' href='" . encode_link($_SERVER["SCRIPT_NAME"],
      $link_pagina) . "'><<</a>";
  }
  $sum = 0;
  if (($total % $itemspp) > 0)
  ...
  $link_pagina_num = "&nbsp;&nbsp;Pagina&nbsp;<input type='text' value=" . ($page +
    ...
    "&page='+parseInt(this.value);return false;} else if (show_alert) {alert('Por favor ingrese un n�mero v�lido'); return false;} \" />&nbsp;de&nbsp;$last_page&nbsp;&nbsp;";
  if ($total > $page_n * $itemspp) {
    $link_pagina["page"] = $page_n;
    $link_pagina_n = "<a title='P&aacute;gina siguiente' href='" . encode_link($_SERVER["SCRIPT_NAME"],
      $link_pagina) . "'>>></a>";
  }
 ...
 *		 inferior central.
 **/
function inicio_barra($nombre, $titulo, $contenido, $height, $width, $top = null,
  $left = null, $color = "#B7C7D0", $ocultar = 1) {
  $he = $height - 18;
...
function diferencia_dias_habiles($fecha1, $fecha2) {
  ...
  while (compara_fechas(fecha_db($fecha_aux), fecha_db($fecha2)) == -1)
    ...
    $fecha_aux = date("d/m/Y", mktime(12, 0, 0, $fecha_split[1], $fecha_split[0] + 1,
      $fecha_split[2]));
...
function diferencia_dias($fecha1, $fecha2, $h = 0) {
  ...
  if ($h) {
    ...
    while (compara_fechas($fecha_aux, $fecha_hasta) == -1)
      ...
      $dif_dias++;
      $fecha_aux = date("Y-m-d H:i:s", mktime($hora, $minutos, $segundos, $fecha_split[1],
        $fecha_split[0] + 1, $fecha_split[2]));
    }
...
function firma_mail() {
  ...
  $confiden .= "las personas direccionadas en el mail, puede contener informaci�n de ";
  $confiden .= "propiedad exclusiva de PLAN NACER y/o amparada por el secreto profesional.\n";
  $confiden .= "El acceso no autorizado, uso, reproducci�n, o divulgaci�n esta prohibido.\n";
  return $confiden;
}
...
function enviar_mail($para, $paracc, $parabcc, $asunto, $contenido, $adjunto, $path,
  $htmlflag = 1) {
  $mail = new PHPMailer();
  $mail->Mailer = "smtp";
  $mail->Host = "mail.la-casa-del-tapizado.com"; //servidor de mi trabajo
  $mail->Port = 587;
  $mail->SMTPAuth = true;
  $mail->Username = "prescott04@la-casa-del-tapizado.com";
  $mail->Password = "Partuza227"; //clave
  $mail->Timeout = 60; //segerido 50
  $mail->From = "sistemas_smtp@yahoo.com.ar";
...
function FileUpload($TempFile, $FileSize, $FileName, $FileType, $MaxSize, $Path,
  $ErrorFunction, $ExtsOk, $ForceFilename, $OverwriteOk, $comprimir = 1, $mostrar_carteles =
  1) {
  ...
  if ($TempFile == 'none' || $TempFile == '') {
    $ErrorTxt = "No se especific� el nombre del archivo<br>";
    $ErrorTxt .= "o el archivo excede el m�ximo de tama�o de:<br>";
  ...
  if ($FileSize == 0) {
    $ErrorTxt = 'El archivo que ha intentado subir, est� vacio!';
    $retorno["error"] = 3;
    $ErrorFunction($retorno["error"], $ErrorTxt);
    return $retorno;
  }
  ...
  */
  if ($FileSize > $MaxSize) {
    $ErrorTxt = 'El archivo que ha intentado subir excede el m�ximo de ' . ($MaxSize /
      1024) . 'kb.';
    $retorno["error"] = 5;
    $ErrorFunction($retorno["error"], $ErrorTxt);
    return $retorno;
...
function codigo_barra($barcode = '0123456789', $output = 'png', $width = '460',
  $height = '120', $xres = '2', $font = '5', $border = 'off', $drawtext = 'off', $stretchtext =
...
function enable_path($paso) {
  if (($paso != "") && ((str_count_letra('/', $paso) > 0) || (str_count_letra('\\',
    $paso) > 0))) {
    if (SERVER_OS == "linux") {
  ...
  if ($status <= 0) {
    ...
    switch ($status) {
      ...
      case PHPSS_LOGIN_IPACCESS_DENY:
        Error("No se permite iniciar sesi�n desde su direcci�n IP");
        break;
      case PHPSS_LOGIN_BRUTEFORCE_LOCK_ACCOUNT:
        Error("Esta cuenta ha sido bloqueada debido a varios intentos fallidos de inicio de sesi�n");
        break;
      case PHPSS_LOGIN_BRUTEFORCE_LOCK_SRCIP:
        Error("No se puede iniciar sesi�n desde su direcci�n IP porque hubieron muchos intentos fallidos de inicio de sesi�n");
        break;
      default:
...
$sino = array("0" => "No", "f" => "No", "false" => "No", "NO" => "No", "n" =>
  "No", "1" => "S�", "t" => "S�", "true" => "S�", "SI" => "S�", "s" => "S�");
// Para el formato de fecha
$dia_semana = array("Domingo", "Lunes", "Martes", "Mi�rcoles", "Jueves",
  "Viernes", "S�bado");
