...
if ($_POST['guardar'] == "Guardar Factura") {
  ...
  $sql = "select cuie, com_gestion, fecha_comp_ges, fecha_fin_comp_ges from nacer.efe_conv where cuie='$cuie'";
...
FROM
  ...
  $traba = $res_factura->fields['traba'];
	...
	//habilitar para controles
  	//$inhabil = controlado_si_no($id_factura);
	$inhabil = 'disabled';
}
...
FROM
  ...
  where factura.id_factura=$id_factura
    ...
    <div style="display:none;width:98%;overflow:auto;<? if ($log->RowCount() > 3)
    echo 'height:60;' ?> " id="tabla_logs" >
                ...
                <tr>
                  ...
                  <td align="left">			 	
                    <select name=cuie Style="width:450px" 
                      ...
                      if (es_cuie($_ses_user['login']))
                        $sql = "select cuie, nombreefector, com_gestion from nacer.efe_conv n inner join facturacion.smiefectores s on n.cuie=s.cuie where n.cuie='$user_login1' order by nombreefector";
                      else {
                        echo"<option value=-1>Seleccione</option>";
                        $sql = "select cuie, nombreefector, upper(trim(com_gestion)) as com_gestion from nacer.efe_conv n inner join facturacion.smiefectores s on n.cuie=s.cuie order by nombreefector";
                      }
                      ...
                      while (!$res_efectores->EOF) {
                        ...
                        echo "selected" ?> Style="background-color: <?=$color_style?>;"><?= $cuiel . " - " . $nombre_efector ?></option>
                                <?
                                $res_efectores->movenext();
                              }
                              ?>
                    </select><button onclick="window.open('../inscripcion/busca_efector.php?qkmpo=cuie','Buscar','dependent:yes,width=900,height=700,top=1,left=60,scrollbars=yes');">b</button>
                  </td>
                </tr>
                <tr>
                  <td align="right">
                  ...
                  <td align="left">
                    ...
                    <input type=text id=fecha_factura name=fecha_factura value='<?= fecha($fecha_factura); ?>' size=15 <? if ($id_factura)
                      echo "readonly" ?>>
                  ...
                  <td align="left">
                    ...
                    <select name=periodo Style="width:450px" <?= $disabled ?>>
                      ...
                      while (!$result->EOF) {
                        ...
                        <option value=<?= $result->fields['periodo'] ?> <? if ($periodo == $result->fields['periodo'])
                        echo "selected" ?>><?= $result->fields['periodo'] ?></option>
                                <?
                                $result->movenext();
                              }
                              ?>			  
                  ...
                  <td align="left">
                    ...
                    <select name=periodo_actual Style="width:450px" <?= $disabled ?>>
                      ...
                      while (!$result->EOF) {
                        ...
                        <option value=<?= $result->fields['periodo'] ?> <? if ($periodo_actual == $result->fields['periodo'])
                        echo "selected" ?>><?= $result->fields['periodo'] ?></option>
                                <?
                                $result->movenext();
                              }
            ...
            <td align="center" colspan="2" class="bordes">
              <? if ($estado == 'A') { ?>
                ...
                <input type="submit" name="anula_factura" value="Anula Factura" onclick="return confirm('Esta Seguro que Desea ANULAR la FACTURA?')" style="width:150px"> 
				 <button onclick="window.open('controles.php?cuie=<?=$cuie?>&id_factura=<?=$id_factura?>','Buscar','dependent:yes,width=900,height=700,top=1,left=60,scrollbars=yes');" <?= $inhabil ?> style="font-size:12px">Correr Controles</button>
              <? } ?> 
              <? if ($estado == 'C') {
                ($traba == 'si') ? $disabled = "disabled" : $disabled = "" ?>
...
<? $link = encode_link("ver_estimulos.php", array("id_factura" => $id_factura));
echo "&nbsp&nbsp<a target='_blank' href='" . $link . "' title='Ver Estímulos'><IMG src='$html_root/imagenes/anotador1.jpg' height='35' width='35' border='0'></a>"; ?>
              </td>
            </tr>
...
<? if ($estado == 'A') { ?>
            ...
            <tr>
              <td align="center" colspan="2" class="bordes" bgcolor="#d3d3cd">		
                ...
                echo "<br><a target='_blank' href='" . $link . "' title='Debito/Credito'><IMG src='$html_root/imagenes/logo_impresora.gif' height='35' width='35' border='0'></a>"; ?>
                <? $link = encode_link("debito_excel1.php", array("id_factura" => $id_factura, "estado" => 'A'));
                echo "&nbsp&nbsp<a target='_blank' href='" . $link . "' title='Genera Excel'><IMG src='$html_root/imagenes/excel.gif' height='35' width='35' border='0'></a>"; ?>
              </td>
            </tr>
