...
if ($_POST['guardar']=="Guardar Prestacion"){
		...
		$id_tema=$result_id_tema->fields['id_categoria_prestacion'];
 		if (($pagina_viene=='comprobante_admin_total.php')||(valida_prestacion3($id_comprobante,$id_tema))){
		    ...
		    $db->CompleteTrans();   
		    $accion="Se Grabo la Prestacion: $id_prestacion.";
 		}
   }
...
if ($pagina_viene=='comprobante_admin_total.php'){
	...
	if ($res_comprobante->recordcount()==0){
		...
		$res_comprobante=sql($sql, "Error al traer los Comprobantes") or fin_pagina();
		$id_smiafiliados=$id;
		$clave_beneficiario=trim($res_comprobante->fields['clave_beneficiario']);
	}
	...
	$sexo=trim($res_comprobante->fields['f']);  
        ...
        /*****
        //Consulta para cargar el combo de las prestaciones
        *****/
        
        if($res_efectores==""){
            $sql= "SELECT * FROM nomenclador.grupo_prestacion WHERE (tema = 'PRESTACION') order by id_categoria_prestacion";
	 ...
	 <table class="bordes" align="center" width="80%">
					    ...
					    <tr>      
					     		...
					     		<select name='prestacion' Style="width:400px"
				                	...
				                	<?	while (!$res_efectores->EOF){ 
				                 		...
				                 		$codigo=$res_efectores->fields['codigo'];?>		                 
				                   		<option value='<?=$codigo;?>' <? if ($prestacion==$codigo) echo "selected"?>> 
					 	...
					 	<tr>
					    	...
					    	<td align="left">
			     				...
			     				<option value=-1>Seleccione</option>
                                                        <? 
                                                        $sql3= "SELECT * FROM nomenclador.grupo_prestacion WHERE (tema = 'OBJETO DE LA PRESTACION' and categoria_padre= '$prestacion' and id_nomenclador_detalle='$id_nomenclador_detalle') order by codigo";
                                                        $res_efectores=sql($sql3) or fin_pagina();
                                                        while (!$res_efectores->EOF){ 
                                                                $categoria=$res_efectores->fields['categoria'];
                                                                $codigo=$res_efectores->fields['codigo'];         			                 	
                                                                ?>
					 	...
					 	<tr>
					    	...
					    	<td align="left">	
					    		<?if ($tipo_diag_radio=='')$tipo_diag_radio='Frec'?>				    		
                                                    <input type="radio" name="tipo_diag_radio" value="Frec" onclick="" <?if ($tipo_diag_radio=='Frec') echo "checked"?></input> Frecuente
					 			<input type="radio" name="tipo_diag_radio" value="Comp" onclick="" <?if ($tipo_diag_radio=='Comp') echo "checked"?></input> Completo
			      			</td>
					    	...
					    	<td align="left">
					     		<select name=patologia Style="width:400px"
									...
									if($prestacion=='R+R'){
										...
										if ($grupo_etareo=='Cero a Cinco A�os'){
											$sql= "SELECT * 
													FROM $tabla_consulta_diag 
													where 
														(id_nomenclador_detalle='$id_nomenclador_detalle') and
										...
										if ($grupo_etareo=='Seis a Nueve A�os'){
											$sql= "SELECT * 
													FROM $tabla_consulta_diag
													where 
														(id_nomenclador_detalle='$id_nomenclador_detalle') and
		 ...
		 <tr> 
		  	<td align="center" colspan="4" class="bordes">
		  		<?php
					if($prestacion!='R+R'){
                                            // Busca los links de formularios correspondientes en la tabla nomenclador.trz_pres
                                            // para mostrar en el boton Detalles
					 $query_trz="SELECT *
							...
							$query_trz="SELECT *
								FROM nomenclador.trz_pres
								WHERE obj_prestacion_vincula='$tema' and grupo_etareo='-' and sexo='-'";	
		    	...
		    	if (($result_trz->RecordCount()>0)&&($result_trz->fields['obliga_efector']=='1')){
		    		if (es_cuie($_ses_user['login']))$hab_on_line="disabled";		    		
		    	}?>
		    	...
		    	if ($result_trz->RecordCount()>0){
		    		...
		    		$diagnostico=$result_trz->fields['diagnostico'];
                                ...
                                /*****
                                 * Consulta que discrimina el link para los 2 distintos formularios de R+R traidos de nomenclador.trz_pres
                                 */
                                
                                if($obj_prestacion_vincula=='R+R01'){
                                    $sql_tmp="select * from (select nacer.smiafiliados.id_smiafiliados as id,
                                        ...
                                        from uad.beneficiarios   left join trazadoras.clasificacion_remediar on beneficiarios.clave_beneficiario=clasificacion_remediar.clave)as cc
                                        WHERE (c ILIKE '%$afidni%' AND a ILIKE '%$afiapellido%')";
                                        
                                        //consulta si no existe ninguna clasificacion con el anterior formulario
                                        $result_aux = sql($sql_tmp) or die;
                                        if(rtrim($result_aux->fields['g'])==''){                                    
                                            //si no existe usa el segundo link de la tabla (el nuevo form)
                                                $result_trz->movenext();
                                        }
                                        
                                }
                                // busca el link al formulario
                                        $pagina_destino=$result_trz->fields['pagina_destino'];
                                //Crea boton detalle
                                $ref = encode_link("../trazadoras/$pagina_destino",array("fecha_comprobante"=>$fecha_comprobante,"id_smiafiliados"=>$id_smiafiliados,"cuie"=>$cuie,"pagina"=>"prestacion_admin.php","tema"=>$tema,"id_nomenclador_detalle"=>$id_nomenclador_detalle,"cuie"=>$cuie,"clave_beneficiario"=>$clave_beneficiario,"fecha_nacimiento"=>$fecha_nacimiento,"sexo_codigo"=>$sexo_codigo,"edad"=>$edad,"prestacion"=>$prestacion,"patologia"=>$patologia,"profesional"=>$profesional,"pagina_viene"=>$pagina_viene,"id_comprobante"=>$id_comprobante));
  	    		?>	
  	    		&nbsp;&nbsp;&nbsp;&nbsp;	    		
				<input type=button name="carga_trazadora" value="<?php echo $texto_boton?>" 
  	    		onclick="if(document.all.patologia.value=='-1'){
							...
							if(document.all.prestacion.value!='R+R'){
 ...
 <tr><td><table width=100% align="center" class="bordes">
  <tr align="center">
   <td>
     ...
     if ($pagina_viene=='comprobante_admin_total.php'){
	 	 $ref = encode_link("comprobante_admin_total.php",array("id"=>$id,"pagina_listado"=>$pagina_listado,"pagina_viene"=>"prestacion_admin.php","estado"=>$estado,"entidad_alta"=>$entidad_alta)); ?>
