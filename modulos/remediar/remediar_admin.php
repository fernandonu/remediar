...
if ($_POST['guardar_editar']=="Guardar"){
	...
	if(rtrim($_POST['factorriesgo'])!=''){ $factorriesgo=$_POST['factorriesgo']; }else{ $accion="No se pudo calcular el Factor de Riesgo.";
    ...
    $db->StartTrans();
        ...
        if ($val->RecordCount()!=0){
            ...
            $query2="update remediar.formulario set factores_riesgo=$factorriesgo,hta2=$hta2,hta3=$hta3,colesterol4=$colesterol4,colesterol5=$colesterol5,dmt26=$dmt26,dmt27=$dmt27,ecv8=$ecv8,tabaco9=$tabaco9,puntaje_final=$puntaje_final
                ,apellidoagente=upper('$apellidoagente'),nombreagente=upper('$nombreagente'),centro_inscriptor=upper('$cuie'),dni_agente=upper('$num_doc_agente'),os=upper('$os'),
				cual_os=upper('$cual_os')
            where id_formulario=$Id_formulario";

            sql($query2, "Error al verificar Remediar2") or fin_pagina();
	
		$query3="update uad.beneficiarios set estado_envio='p',score_riesgo=$puntaje_final
	    ...
	    $val=sql($query, "Error en consulta de validacion") or fin_pagina();
            ...
            if ($val->RecordCount()==0){
                ...
                $query2="insert into remediar.formulario (id_formulario,nroformulario,factores_riesgo,hta2,hta3,colesterol4,colesterol5,dmt26,dmt27,ecv8,tabaco9,puntaje_final,apellidoagente,nombreagente,centro_inscriptor,dni_agente,os,cual_os)
                        values($Id_formulario,$num_form_remediar,$factorriesgo,$hta2,$hta3,$colesterol4,$colesterol5,$dmt26,$dmt27,$ecv8,$tabaco9,$puntaje_final,upper('$apellidoagente'),upper('$nombreagente'),upper('$cuie'),upper('$num_doc_agente'),upper('$os'),upper('$cual_os'))";
         ...
         <tr>
             ...
             <tr id="mo">
                <td align="center" colspan="4" >
                    <b> N&uacute;mero de Formulario Remediar + Redes </b><input type="text" maxlength="10" name="num_form_remediar" value="<?=$num_form_remediar?>" <?php if ($num_form_remediar){ echo 'readOnly onclick="nuevo_remediar();"'; } ?>><input type=submit name="buscar_remediar" value="b" title="b" >
               </td>
             </tr>
             <tr id="ma">
