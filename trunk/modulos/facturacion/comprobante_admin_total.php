		...
		/*	$q="select nextval('comprobante_id_comprobante_seq') as id_comprobante";
		  ...
		  */  
		    $query="insert into facturacion.comprobante
		       (id_comprobante, cuie, nombre_medico, fecha_comprobante, clavebeneficiario, id_beneficiarios, fecha_carga,periodo,comentario,id_servicio,activo,entidad_alta)
		             values
		      (nextval('comprobante_id_comprobante_seq'),'$cuie','$nom_medico','$fecha_comprobante','$clavebeneficiario', $id,'$fecha_carga','$periodo','$comentario','$servicio','N','$entidad_alta') RETURNING id_comprobante";	
		    ...
		    if ($_POST['guardar']=="Guardar Comprobante y Facturar"){
	 			$ref = encode_link("prestacion_admin.php",array("id"=>$id,"id_comprobante"=>$id_comprobante,"pagina_viene"=>"comprobante_admin_total.php","pagina_listado"=>$pagina_listado,"entidad_alta"=>$entidad_alta));
  ...
  <table width=70% align="center" class="bordes">
	 ...
	 <table class="bordes" align="center" width="70%">
		 <tr align="center" id="sub_tabla">
		 	<td colspan="2">	
                            Nueva Prestaci&oacute;n No Empadronado		 		
		 	</td>
		 </tr>
					 ...
					 <tr>
					    ...
					    <td align="left">		          			
			                 ...
			                 if (es_cuie($_ses_user['login']))
			                 	$sql= "select cuie, nombre, upper(trim(com_gestion)) as com_gestion from nacer.efe_conv n inner join facturacion.smiefectores s on n.cuie=s.cuie where cuie='$user_login1' order by nombreefector";	
			                 else{
			                 	echo "<option value=-1>Seleccione</option>";
			                    $sql= "select s.cuie, nombreefector, upper(trim(com_gestion)) as com_gestion from nacer.efe_conv n inner join facturacion.smiefectores s on n.cuie=s.cuie order by nombreefector";
			                 }
	 ...
	 <?}
	 	...
	 	while (!$res_comprobante->EOF) {
	 		if ($res_comprobante->fields['id_factura']==""){
	 			$color_fondo="#FFFFCC"; 		
	 			$ref = encode_link("prestacion_admin.php",array("id"=>$id,"id_comprobante"=>$res_comprobante->fields['id_comprobante'],"pagina_viene"=>"comprobante_admin_total.php","pagina_listado"=>$pagina_listado,"entidad_alta"=>$entidad_alta));
            	$onclick_elegir="location.href='$ref'";  
            	if ($res_comprobante->fields['marca']==0){
            		$ref1 = encode_link("comprobante_admin_total.php",array("id_comprobante"=>$res_comprobante->fields['id_comprobante'],"marcar"=>"True","id"=>$id));
