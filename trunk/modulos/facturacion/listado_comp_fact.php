...
if($_POST["vincular_todo"]=="Vincular Todo"){
   ...
   if (es_cuie($_ses_user['login'])&&($res_parametro=='si')){
	...
	$query.=" where comprobante.id_factura is null and comprobante.cuie='$cuie' and marca=0 and fecha_comprobante between '$fecha_desde' and '$fecha_hasta' and (comprobante.activo='S' or comprobante.activo is NULL) and ((comprobante.fecha_comprobante + '$res_parametro1 days') > '$res_fecha_factura') ";
   }
   else{
...
if (es_cuie($_ses_user['login'])&&($res_parametro=='si')){
	...
	$fecha_desde=ereg_replace('/','-',$periodo_actual).'-01';	
	$fecha_hasta=ereg_replace('/','-',$periodo_actual).'-'.ultimoDia($mes,$anio);	
	$where_tmp=" comprobante.id_factura is null and comprobante.cuie='$cuie' and marca=0 and fecha_comprobante between '$fecha_desde' and '$fecha_hasta' and (comprobante.activo='S' or comprobante.activo is NULL) and ((comprobante.fecha_comprobante + '$res_parametro1 days') > '$res_fecha_factura') ";
	$accion="Muestra SOLO los comprobantes del PERIODO PRESTACION de la Factura";
}
else{
...
<table border=0 width=100% cellspacing=2 cellpadding=2 bgcolor='<?=$bgcolor3?>' align=center>
  <tr>
     ...
     <table width=100%>
      <tr id=ma>
       <td width=30% align=left><b>Total:</b> <?=$total_muletos?>&nbsp;&nbsp; <input type="submit" value="Vincular Todo" name="vincular_todo" onclick="return confirm ('¿Esta seguro que desea Vincular TODO?')"></td> 
  ...
  <tr>
    ...
    <td align=right id=mo><a id=mo href='<?=encode_link("listado_comp_fact.php",array("sort"=>"4","up"=>$up,"cuie"=>$cuie))?>'>Tipo Beneficiario</a></td>
    <td align=right id=mo><a id=mo href='<?=encode_link("listado_comp_fact.php",array("sort"=>"5","up"=>$up,"cuie"=>$cuie))?>'>Fecha Prestación</a></td>       
    <td align=right id=mo><a id=mo href='<?=encode_link("listado_comp_fact.php",array("sort"=>"7","up"=>$up,"cuie"=>$cuie))?>'>Clave Beneficiario</a></td>
 ...
 <?
   while (!$result->EOF) {
    ...
    <tr <?=atrib_tr()?>>
     ...
     <td onclick="if (confirm ('Esta Seguro que desea Vincular el Comprobante seleccionado')){<?=$onclick_elegir?>}"><?=$result->fields['descripcion']?></td> 
     ...
     <td onclick="if (confirm ('Esta Seguro que desea Vincular el Comprobante seleccionado')){<?=$onclick_elegir?>}"><?=$result->fields['clavebeneficiario']?></td> 
